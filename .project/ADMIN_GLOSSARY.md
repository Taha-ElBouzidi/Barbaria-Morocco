...
## Story intro
Two or three sentences that set the scene for the buyer at the top of the box detail page. Narrative voice. Example: "A complete hammam ritual composed for spas seeking the house's blue-hour signature."
...
A special box per category. Buyers who land on this box's URL get the immersive wizard instead of a pre-set box. Currently the house has two of these: `compose-cosmetiques` and `compose-epicerie`. Curated (pre-set) boxes leave this unchecked.
...
## Ritual label
A short label that appears as an eyebrow above the product name. Example: "Or Liquide du Maroc" (for argan oil), "Signature Barbaria" (for the house's flagship). Optional but evocative.
...
The "how to use" ritual that used to appear on product pages. **Removed from the product editor UI** because products no longer surface individually on the public site after the box-first IA shift. Existing rows in `product_application_steps` are preserved in the DB so they can be brought back if the house decides to publish how-to-use content later.

## Occasion
The Occasion dropdown on the public contact form. Editable from `/admin/occasions` so the house can surface seasonal events (Mother's Day, Eid, Christmas) without a code deploy.
